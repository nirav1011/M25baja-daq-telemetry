...
off-road race car. A Teensy microcontroller reads an IMU over SPI and a GPS module
...
```mermaid
flowchart LR
    IMU["IMU<br/>orientation + accel"] -- SPI --> T["Teensy MCU<br/>sensor drivers,<br/>sampling loop"]
...
**Acquisition (Teensy).** Drivers poll the IMU over SPI and parse the GPS stream over
...
**Mine:**
- IMU driver over SPI and GPS driver over UART on the Teensy
...
```
firmware/        Teensy — IMU (SPI) and GPS (UART) drivers, sampling loop, framing
...
- **Timestamp discipline.** Sensor timestamps come from the Teensy's clock with no
...
- **Framing robustness.** The serial protocol between Teensy and Pi has no sequence
